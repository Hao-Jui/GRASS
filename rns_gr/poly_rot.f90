...
#include "option_macro.h"
  ...
  character(32) :: fil1, fil2, fil3, fil4, fil5
  ! functions
  real(8) :: deriv_s, deriv_m, deriv_sm
  external rotation_law_poly, diff_rotation_poly
  ...
  do while( dif > accuracy .or. n_of_it <2 )
    ...
    if (r_e_new .ne. r_e_new .or. r_e_new/r_e_old > 2) then
      write(*,*) " ",gama_pole_h, rho_pole_h, gama_center_h, rho_center_h
    ...
    if(r_ratio == 1.0) then
      ...
      call zbrent_diff(Omega_e*8.d-1,r_e_new,rho_equator_h,gama_equator_h, &
                       ww_equator_h,rho_pole_h,gama_pole_h, 0.d0, 1.d-5, Omega_e,  diff_rotation_poly)

      term_in_Omega_h = abs(Omega_e-ww_equator_h) * exp( -2.d0 * r_e_new_sq * rho_equator_h )
      Omega_c = Omega_e + ( term_in_Omega_h / ( 1.d0 - term_in_Omega_h * abs(Omega_e-ww_equator_h) ) / parA )**(1.d0/parB)
  ...
  !!! compute Omega profile
      ...
      do s = 2, 2*SDIV/3
        do m = 1, MDIV-1
          ...
          mum = mu(m)
          sgp = s_gp(s)
          call zbrent_rot(Omg(s-1,m)*8.d-1, r_e_new,rsm,wwsm,sgp,mum, 1.d-5, omg(s,m), rotation_law_poly)
    ...
    do s = 1, SDIV
      ...
      do m = 1, MDIV
        ...
        enthalpy(s,m) = enthalpy_min + 5.d-1 * ( &
              ...
              + parA / (parB+1.d0) * (r_e_new*abs(Omega_c-omg(s,m)))**(parB+1.d0)
        if ( enthalpy(s,m) .ne. enthalpy(s,m) ) then
          write(*,"(12es15.6)") log(1.d0-velocity_sq(s,m)), (Omega_c-omg(s,m))**(parB+1.d0), &
              gama_pole_h + rho_pole_h - gama(s,m) - rsm
          stop "L.144 of poly_rot"
        endif
...
! --- End of iteration 
  ...
  if (output) then
    ...
    write(fil1,"(es15.3)") ang_mom
    write(fil2,"(f16.5)") mass_0/MSUN
    write(fil3,"(es15.3)") rho_0
    write(fil4,"(f6.4)") parA 
    write(fil5,"(f6.4)") parB
    open(98,file="./Cont/"//trim(adjustl(eos_file))//"_J_"//trim(adjustl(fil1))//&
        "_Mb"//trim(adjustl(fil2))//"_rhoc"//trim(adjustl(fil3))//&
        "_parA"//trim(adjustl(fil4))//"_parB"//trim(adjustl(fil5))//".dat")
...
subroutine diff_rotation_poly(x, fx, re,rho_h, g_h, w_h, rho_p, g_p, dh)
  use para_mod, only: parA, parB
  implicit none
  real(8), intent(in) :: x, re, rho_h, g_h, w_h, rho_p, g_p, dh
  real(8), intent(out):: fx
  real(8) :: LHS

  LHS =  re**2 * (g_h + rho_h - g_p - rho_p) + log(1.d0 - ((x-w_h)*exp(-re**2*rho_h))**2 )
  fx = LHS * (1.d0 - ((x-w_h)*exp(-re**2*rho_h))**2 )**(1.d0+1.d0/parB) * ( 1.d0 + parB ) &
               - ( (x-w_h) * re * exp(-2.d0*re**2*rho_h) )**(1.d0+1.d0/parB) / parA**(1.d0/parB) * 2.d0
      
end subroutine diff_rotation_poly

subroutine rotation_law_poly(x, fx, re, rho_p, ww_p, sgp, mugp)
  use para_mod, only: parA, parB, Omega_c
  implicit none
  real(8), intent(in) :: x, re, rho_p, ww_p, sgp, mugp
  real(8), intent(out):: fx

  fx = ( Omega_c - x ) * ( (1.d0-sgp)**2 - ( sgp*(x-ww_p)*exp(-re**2*rho_p) )**2 * (1.d0-mugp*mugp) )** (1.d0/parB) &
      - (( (x-ww_p) * sgp * sgp * (1.d0-mugp*mugp) * exp(-2.d0 * re**2 * rho_p) ) / parA)** (1.d0/parB)
      
end subroutine rotation_law_poly
