...
contains
  ...
  subroutine relaxation(target_rho, target_gama, target_ww, target_sphi, root_mphi_re, n_of_it, dif)
    ...
    real(wp), parameter :: DIF_TAIL_THRESH = 1.e-5_wp
    ...
    integer,  parameter :: N_CYCLE_LAGS = 4, CYCLE_LAG_MAX = 200
    integer,  parameter :: CYCLE_LAGS(N_CYCLE_LAGS) = [50, 100, 150, 200]

    real(wp), save :: cheb_w = 1.e0_wp, rho_spec_est = 0.7e0_wp, rho_spec_prev = 0.7e0_wp
    real(wp), save :: prev_dif_local = -1.e0_wp, dif_prev = -1.e0_wp
    real(wp), save :: picard_best_dif = huge(1.e0_wp), w_mix = W_PICARD
    real(wp), save :: dif_min_seen = huge(1.e0_wp), dif_ring(CYCLE_LAG_MAX) = -1.e0_wp
    integer,  save :: n_consec_decrease = 0, n_picard_stall = 0, n_rho_locked = 0
    integer,  save :: n_cheb_hurt = 0, n_anderson_metric = 0, n_anderson_scalar = 0
    integer,  save :: n_aitken_cooldown = 0, n_anderson_cooldown = 0, ring_pos = 0
    logical,  save :: last_was_cheb = .false., last_was_aitken = .false.
  ...
  contains
    ...
    subroutine ensure_allocated
      ...
      cheb_w = 1.e0_wp; rho_spec_est = 0.7e0_wp; rho_spec_prev = 0.7e0_wp
      prev_dif_local = -1.e0_wp; dif_prev = -1.e0_wp
      ...
      last_was_cheb = .false.; last_was_aitken = .false.
      picard_best_dif = huge(1.e0_wp); w_mix = W_PICARD
      dif_min_seen = huge(1.e0_wp); dif_ring = -1.e0_wp; ring_pos = 0
    ...
    subroutine update_diagnostics
      ...
      if (dif < DIF_TAIL_THRESH) then
        do lag_idx = 1, N_CYCLE_LAGS
          ...
          if (dif_ref > 0.e0_wp .and. abs(dif - dif_ref) <= CYCLE_TOL * max(dif, dif_ref)) then
            cycle_hit = .true.; exit
          end if
        end do
      end if
      if (cycle_hit) then
        ...
        n_anderson_metric = 0; n_anderson_scalar = 0
        hist_rho = 0.e0_wp; hist_gama = 0.e0_wp; hist_ww = 0.e0_wp; hist_sphi = 0.e0_wp
      end if
    ...
    subroutine reset_picard_state
      n_picard_stall = 0; picard_best_dif = huge(1.e0_wp); w_mix = W_PICARD
    ...
    subroutine enforce_sphi_floor
      ...
      do m = 1, MDIV
        do s = 2, SDIV
          if (sphi(s,m) < 0.e0_wp) sphi(s,m) = sphi(s-1,m) * sphi_floor_decay(s)
        end do
      end do
