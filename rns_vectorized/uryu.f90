...
#include "option_macro.h"
  ...
  !!! Compute new r_e; consider location at the maximum of energy density
    ...
    imax    = maxloc( ee_mu_0, 1 )
    p       = p_at_e(ee_mu_0(imax))
    if (imax > SDIV / 2) then
      open(12,file="./Cont/ee_mu_0.dat")
      do s = 1, SDIV
        write(12,"(10es18.9)") s_gp(s), ee_mu_0(s)
      enddo
      close(12)
      stop "please check the profile of ./Cont/ee_mu_0.dat; L76 in Uryu"
    endif
    ...
    do while( abs(diff_Fmax) > 1.d-7)
        ...
        call zbrent_diff(ax, r_e_new,rho_equator_h,gama_equator_h,ww_equator_h, &
                        rho_pole_h,gama_pole_h, 0.d0, 1.d-5, fa, diff_rotation_uryu) ! Fmax_h used here
        Omega_e = fa
        ...
        !write(*,"(30es15.6)") Fmax_h, diff_Fmax
        Fmax_h    = Fmax_h - diff_Fmax * 1.d-2
    enddo
    !stop
    !write(*,"(5es18.9)") Omega_e, Fmax_h
...
!!! Compute velocity, energy density and pressure
  ...
  if (output) then
    write(*,"(A21)") " ", "+++++ Data for FUKA :"
    write(*,"(4es18.9)") AA_h(F_equator_h,Fmax_h)*r_e/(Omega_c* (C/sqrt(kappa))/f_uni), &
                         BB_h(F_equator_h,Fmax_h)*r_e/(Omega_c* (C/sqrt(kappa))/f_uni), &
                         AA_h(F_equator_h,Fmax_h) / BB_h(F_equator_h,Fmax_h),(Omega_c* (C/sqrt(kappa))/f_uni)
    call mass_radius  
...
subroutine diff_rotation_uryu(x, fx, re, rho_h, g_h, w_h, rho_p, g_p, dh)
! Compute Omega_e
...
real(8) function intF(x, F_at_x)
  ...
  if ( x == 0.d0 .and. F_at_x == 0.d0 ) then
    ...
    intF = F_at_x * x - aa * omega_c / 4.d0 * &
        (2.d0 * aa / bb * atan( tmp**2 ) &
        - sqrt(2.d0) * ( atan(1.d0-sqrt(2.d0)*tmp) - atan(1.d0+sqrt(2.d0)*tmp) ) &
