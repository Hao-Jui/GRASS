...
#include "option_macro.h"
  use toolkit_mod, only: deriv_s, deriv_m, deriv_sm, interp
  use para_mod
  implicit none
  integer :: m, s
  integer :: n, k, n_of_it
  ...
  do while( dif > 1.d-7 .or. n_of_it <2 )
      ...
      do s = 1, SDIV
        ...
        do m = 1, MDIV
          ...
          if (r_ratio == 1.d0) then
              ...
              velocity_sq(s,m) = ( (Omega_c-ww(s,m)) * (sgp/(1.d0-sgp))**s_pwr * sin_theta(m) * exp(-rsm*r_e_new_sq) )**2
          endif
      ...
      do s = 1, SDIV
        do m = 1, MDIV
          ...
          S_metric_omega(s,m) = e_gsm * e_rsm * (-ea * (Omega_c-wwsm) * (esm+psm) * s2 / (1.d0-v2sm) &
            ...
            - s1 * ( 2.d0 * d_rho_s + d_gama_s / 2.d0 ) &
            + mum * ( 2.d0 * d_rho_m + d_gama_m / 2.d0 ) &
            + s1**2 * ( 4.d0 * d_rho_s**2 - d_gama_s**2 ) / 4.d0 &
            + m1 * ( 4.d0 * d_rho_m**2 - d_gama_m**2 ) / 4.d0 &
            - m1 * e_rsm**2 * s2 * ( ( s1 * d_ww_s )**2 + m1 * d_ww_m**2 ) ) )
        enddo
      enddo
    ...
    ! compute first order derivatives of gama
      ...
      !open(76,file="./trig/source.dat")
      !do s = 1, SDIV
      !  do m = 1, MDIV
      !    write(76,"(99es18.9)") s_gp(s), mu(m), S_metric_rho(s,m), S_metric_gama(s,m), S_metric_omega(s,m), &
      !      rho(s,m), gama(s,m), ww(s,m), dgds(s,m), dgdm(s,m)
      !  enddo
      !enddo
      !close(76); stop
      ...
      do s = 2, SDIV
        do m = 1, MDIV
            da_dm(1,m) = 0.d0
            sgp = s_gp(s)
            s1  = sgp * (1.d0-sgp) / dble(s_pwr) ! c_1 in the paper
            mum = mu(m)
            m1  = 1.d0 - mum**2
            d_gama_s  = dgds(s,m)
            d_gama_m  = dgdm(s,m)
            ...
            da_dm(s,m) = - ( d_rho_m + d_gama_m ) / 2.d0 &
              - temp2 * ( (temp3 - d_gama_mm - temp4) * (-mum + m1 * d_gama_m) / 2.d0 & ! OK
