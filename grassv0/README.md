...
General Relativistic Axisymmetric Spacetime Solver (GRASS) computes
rotating neutron-star equilibria in either General Relativity (GR) or
Scalar–Tensor (ST) gravity. The code integrates the field equations on a
...
```bash
mkdir -p Cont
gfortran -O3 -Isrc/core \
  src/main.f90 \
  src/core/para_mod.f90 \
  src/core/toolkit_mod.f90 \
  src/core/simpson_mod.f90 \
  src/core/newton_mod.f90 \
  src/core/shoot_v2.f90 \
  src/core/analysis_v2.f90 \
  -lblas -llapack -lnag \
  -o grass
```

Adjust library flags for your system (e.g., `-lopenblas`, Intel MKL, or
custom NAG paths).
...
- **Alternative rotation laws**: modify `para_mod.f90` defaults or add
  new parameters.
- **EOS handling**: adapt `loadEos` (defined via `option_macro.h` / EOS
  utilities) to support additional formats.
