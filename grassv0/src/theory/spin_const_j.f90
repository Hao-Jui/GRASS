...
subroutine spin_gr_const_j
  ...
  real(8) :: rho_pole_h, rho_center_h, rho_equator_h
  real(8) :: ww_equator_h
  ...
  do while (dif > accuracy .or. n_of_it < 2)
    ...
    if (r_e_new .ne. r_e_new .or. r_e_new / r_e_old > 2) then
      write(*,*) " "
      stop 'change in r_e is too dramatic; L79 in spin_gr_const_j'
    end if
    ...
    if (abs(rho(2,1)) > 100.d0 .or. abs(gama(2,1)) > 300.d0 .or. abs(ww(2,1)) > 100.d0) then
      ...
      stop "Line 360 in spin_gr_const_j"
    end if
...
end subroutine spin_gr_const_j

end module rotation_const_j

subroutine diff_rotation_const_j(x, fx, re, rho_h, g_h, w_h, rho_p, g_p, dh)
