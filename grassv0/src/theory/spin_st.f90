...
#include "option_macro.h"
  ...
  real(8) :: f_sphi, mr_s, mr_k, root_mphi_re
  real(8) :: er2, rho_0
  logical :: valid(MDIV)
  ...
  real(8), allocatable :: mr_cache(:), besseli_cache(:,:), besselk_cache(:,:), wfac_cache(:)
  real(8), allocatable :: expm_mr_cache(:), inv_mr_cache(:)
  real(8) :: exp_arg, exp_val
  logical :: warned_Omega_exp, warned_Omega_sqrt
  ...
  warned_Omega_exp = .false.
  warned_Omega_sqrt = .false.
  r_ratio_const = 1.d0
  ...
  if (.not. allocated(e_gsm_cache)) then
    ...
    allocate(expm_mr_cache(SDIV), inv_mr_cache(SDIV))
  end if
  ...
  do while( dif > 3.d-7 .or. n_of_it < 2 )
    ...
    if (r_ratio_const == 1.d0) then
      ...
      if (term_in_Omega_h >= 0.d0) then
        ...
        stop "Omega can't be found"
      end if
    end if

    ! bessel and radial weights
    root_mphi_re = sqrt(mphi_r*r_e_new_sq)
    do s = 1, SDIV
      mr_cache(s)    = root_mphi_re * s_gp(s) / (1.d0 - s_gp(s))
      wfac_cache(s)  = 1.d0 / (1.d0 - s_gp(s))**2
      if (mr_cache(s) >= 2.3d2) then
        expm_mr_cache(s) = 1.d-99
      else
        expm_mr_cache(s) = max(exp(-mr_cache(s)), 1.d-99)
      end if
      if (abs(mr_cache(s)) > tiny(1.d0)) then
        inv_mr_cache(s) = 1.d0 / mr_cache(s)
      else
        inv_mr_cache(s) = 0.d0
      end if
    end do
    ...
    ! ---------------------------------------------------------------
    ! Compute velocity, EoS (vectorized-style, but no OpenMP)
    ! ---------------------------------------------------------------
    do s = 1, SDIV
      sgp = s_gp(s)
    ...
    ! second derivatives of gama + exponentials & couplings
    do s = 1, SDIV
      do m = 1, MDIV
        sgp = s_gp(s)
        s1 = sgp*(1.d0 - sgp)
    ...
    ! ---------------------------------------------------------------
    ! BUILD SOURCE TERMS using precomputed values
    ! ---------------------------------------------------------------
    S_metric_rho   = 0.d0
    S_metric_sphi  = 0.d0
    S_metric_gama  = 0.d0
    ...
    do m = 1, MDIV
      ...
      do s = 1, SDIV
        ...
        ! exp(2*alpha) replaced by e2alpha_r2_cache / r_e_new_sq
        S_metric_sphi(s,m) = - r_e_new_sq * s2 * scal_p * mphi_r &
          + r_e_new_sq * s2 * scal_p * ( e2alpha_r2_cache(s,m)/r_e_new_sq ) * ( -2.d0 * pi * B_coup * (esm-3.d0*psm) + mphi_r) &
    ...
    do s = 1, SDIV
      do m = 1, MDIV
        ...
        do n = 1, LMAX
          ...
          sum_sphi= sum_sphi- exp(-0.5d0*gsm) * (2.d0*dble(n)+1.d0) * P_2n(m,n+1) * D2_metric_sphi(s,n+1)
          if (m == MDIV) then
    ...
    if (r_ratio_const == 1.d0) then
      ...
      do s = 2, SDIV
        do m = 1, MDIV
          ...
          d_gama_sm = deriv_sm(gama,s,m)  ! mixed derivative (compute on the fly)
          ...
          temp8 = m1 * (e_rsm_cache(s,m)**2)  ! exp(-2*rho) via cache
    ...
    do s = 1, SDIV
      do m = 1, MDIV
        ...
        if(alpha(s,m).ge.300.0) stop "alpha fails"
        ww(s,m) = ww(s,m) / r_e_new
      end do
    end do
