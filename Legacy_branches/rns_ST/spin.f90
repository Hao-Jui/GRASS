subroutine spin
#include "option_macro.h"
use para_mod
  implicit none
  integer :: m, s
  integer :: n, k, n_of_it
  real(8) :: r_p, s_p
  real(8) :: sum_rho,sum_gama,sum_omega,sum_sphi, &  ! intermediate sum
        r_e_old, &                     ! equatorial radius in previus cycle
        dif                            ! difference | r_e_old - r_e |
  real(8) :: d_gama_s,d_gama_m, d_gama_ss,d_gama_mm,d_gama_sm, &
        d_rho_s,d_rho_m,d_ww_s,d_ww_m,d_sphi_s,d_sphi_m
  real(8) :: temp1,temp2,temp3,temp4,temp5,temp6,temp7,temp8,temp9
  real(8) :: Vphi, Acoup, f_sphi, mr_s, mr_k
  real(8) :: besseli, besselk
  real(8) :: m1, s1, s2, ea, mum, sgp, s_1
  real(8) :: rsm, gsm, wwsm, esm, psm, v2sm, e_gsm, e_rsm
  real(8) :: grgr, term_in_Omega_h
  real(8) :: gama_pole_h,gama_center_h,gama_equator_h, &
        rho_pole_h,rho_center_h,rho_equator_h, ww_equator_h
  real(8) :: sphi_pole_h,sphi_center_h,sphi_equator_h,scal_p
  real(8) :: r_e_new,r_e_new_sq
  real(8) :: deriv_s,deriv_m,deriv_sm!,deriv_ss,deriv_mm
  real(8) :: e_at_p,p_at_h,n0_at_e,e_at_h
  real(8) :: rho_0
  integer :: ifail
  real(8) :: er2, Int_m(MDIV), Int_s(SDIV)
  real(8), dimension(SDIV) :: gama_mu_1,gama_mu_0,rho_mu_1,rho_mu_0,ww_mu_0
  real(8), dimension(SDIV,MDIV) :: da_dm, dgds, dgdm
  real(8), dimension(SDIV,MDIV) :: S_metric_rho, S_metric_gama, S_metric_omega, S_metric_sphi
  real(8), dimension(LMAX+1,SDIV) :: D1_metric_rho, D1_metric_gama, D1_metric_omega, D1_metric_sphi
  real(8), dimension(SDIV,LMAX+1) :: D2_metric_rho, D2_metric_gama, D2_metric_omega, D2_metric_sphi
  character(32) :: fil1, fil2, fil3, fil4, fil5, fil6

  dif = 1.d0; n_of_it =0

  r_e_new = r_e; r_e_new_sq = r_e_new**2
  if ( maxval(sphi*sqrt(B_coup)) < 1.d-3 ) sphi = sphi*1.d1

  if ( any(isnan(sphi)) ) stop "NaN found in the sphi profile; L39 of spin"

  do while( dif > 1.d-7 .or. n_of_it < 2 )
    do s = 1, SDIV
      do m = 1, MDIV
        rho  (s,m) = rho  (s,m) / r_e_new_sq ! hat
        sphi (s,m) = sphi (s,m) / r_e_new
        gama (s,m) = gama (s,m) / r_e_new_sq ! hat
        alpha(s,m) = alpha(s,m) / r_e_new_sq ! hat
        ww   (s,m) = ww   (s,m) * r_e_new    ! hat
      enddo
      ! along x-axis
      rho_mu_0 (s) = rho (s,1) ! hat
      gama_mu_0(s) = gama(s,1) ! hat
      ww_mu_0  (s) = ww  (s,1) ! hat
      ! along z-axis
      rho_mu_1 (s) = rho (s,MDIV) ! hat
      gama_mu_1(s) = gama(s,MDIV) ! hat
    enddo

    ! Compute new r_e
    r_e_old = r_e_new ! only to compute dif
    r_p     = r_ratio * r_e_new
    s_p     = r_p / (r_p + r_e_new)

    call interp(s_gp, sphi(:,MDIV), SDIV, s_p, sphi_pole_h)
    sphi_center_h = sphi(1,1)

    call interp(s_gp, gama_mu_1, SDIV, s_p, gama_pole_h)
    call interp(s_gp, gama_mu_0, SDIV, s_e, gama_equator_h)
    gama_center_h = gama(1,1) ! hat

    call interp(s_gp,  rho_mu_1, SDIV, s_p, rho_pole_h)
    call interp(s_gp,  rho_mu_0, SDIV, s_e, rho_equator_h)
    rho_center_h = rho(1,1) ! hat

    grgr = gama_pole_h + rho_pole_h - gama_center_h - rho_center_h &
            + B_coup/2.d0 * ( sphi_center_h**2 - sphi_pole_h**2 ) ! hat
    r_e_new_sq = ( 2.d0 * ( h_center - enthalpy_min ) ) / grgr

    dif = abs(r_e_old-sqrt(r_e_new_sq)) / sqrt(r_e_new_sq)

    !if ( mod(n_of_it+1,50)==0 ) then 
    if ( n_of_it > 300 .and.  mod(n_of_it,50)==0 ) then 
      write(*,"(A8,i5,A20,20es18.9)") "n_or_it =", n_of_it, &
        ",     diff, sphi_c =", dif, sphi_center_h*r_e_new*sqrt(B_coup),gama_center_h, rho_center_h, alpha(1,1)
    endif

    r_e_new = sqrt( r_e_new_sq )
    if (r_e_new/r_e_old > 2) then
        write(*,*) r_e_new, n_of_it
        stop 'change in r_e is too dramatic; L79 in spin'
    elseif (r_e_new .ne. r_e_new) then
        write(*,"(10es18.9)") sphi_center_h, sphi_pole_h
        stop 'nan in r_e_new'
    endif

    ! Compute angular velocity Omega, only for rigid rotation
    if(r_ratio == 1.d0) then
      Omega_c = 0.d0
      ww_equator_h = 0.d0
    else
      call interp( s_gp,   ww_mu_0, SDIV, s_e,   ww_equator_h )
      call interp( s_gp, sphi(s,1), SDIV, s_e, sphi_equator_h ) 
      grgr = gama_pole_h + rho_pole_h - gama_equator_h - rho_equator_h &
            + B_coup/2.d0 * ( sphi_equator_h**2 - sphi_pole_h**2 ) ! hat
      term_in_Omega_h = 1.d0 - exp( r_e_new_sq * grgr ) 
      if (term_in_Omega_h >= 0.d0) then
        Omega_c = ww_equator_h + exp(r_e_new_sq*rho_equator_h) * sqrt(term_in_Omega_h) ! hat
      else
        write(*,"(10es15.6)") Omega_c,term_in_Omega_h,exp(2.d0*(h_center-enthalpy_min))
        stop "Omega can't be found; L104 in spin"
      endif
    endif

    ! Compute velocity, energy density and pressure
    do s = 1, SDIV
      sgp = s_gp(s)
      do m = 1, MDIV
        rsm = rho(s,m) ! hat
        if (r_ratio == 1.d0) then
          velocity_sq(s,m) = 0.d0
        else
          velocity_sq(s,m) = ( (Omega_c-ww(s,m))*(sgp/(1.d0-sgp)) * sin_theta(m) * exp(-rsm*r_e_new_sq) )**2
        endif

        if (velocity_sq(s,m) > 1.d0) velocity_sq(s,m) = 0.d0

        ! enthalpy_min is the assumed small value for the value at the pole
        enthalpy(s,m) = enthalpy_min + 5.d-1 * ( &
              r_e_new_sq * ( gama_pole_h + rho_pole_h - gama(s,m) - rsm &
              + ( sphi(s,m)**2 - sphi_pole_h**2 )*B_coup/2.d0 &
              ) - log(1.d0-velocity_sq(s,m))  )

        if (enthalpy(s,m) <= enthalpy_min .or. sgp > s_e) then
          enthalpy(s,m) = enthalpy_min
          pressure(s,m) = 0.d0
          energy(s,m)   = 0.d0
        else
          pressure(s,m) = p_at_h(enthalpy(s,m))
          energy(s,m)   = e_at_p(pressure(s,m))
        endif
        ! Rescale back metric potentials (except omega)
        rho  (s,m) = rho  (s,m) * r_e_new_sq
        gama (s,m) = gama (s,m) * r_e_new_sq
        alpha(s,m) = alpha(s,m) * r_e_new_sq
        sphi (s,m) = sphi (s,m) * r_e_new
      enddo
    enddo

    do s = 1, SDIV; do m = 1, MDIV
      dgds(s,m) = deriv_s(gama,s,m)
      dgdm(s,m) = deriv_m(gama,s,m)
    enddo; enddo

    ! Compute metric potentials
    S_metric_rho   = 0.d0
    S_metric_sphi  = 0.d0
    S_metric_gama  = 0.d0
    S_metric_omega = 0.d0

    do s = 1, SDIV
      do m = 1, MDIV
        sgp   = s_gp(s)
        Acoup = exp( - sphi(s,m)**2 * B_coup / 4.d0 )
        rsm   = rho     (s,m)
        gsm   = gama    (s,m)
        wwsm  = ww      (s,m) ! hat
        esm   = energy  (s,m) * Acoup**4
        psm   = pressure(s,m) * Acoup**4
        e_gsm = exp( gsm/2.d0 )
        e_rsm = exp(-rsm)
        scal_p= sphi(s,m) * e_gsm
        v2sm  = velocity_sq(s,m)
        mum   = mu(m)
        m1    = 1.d0 - mum**2
        s_1   = 1.d0 - sgp
        s1    = sgp * s_1
        s2    = (sgp / s_1)**2
        ea    = 16.d0 * pi                 * exp(2.d0*alpha(s,m)) * r_e_new_sq
        Vphi  = sphi(s,m)**2 * mphi_ep / 2.d0 * exp(2.d0*alpha(s,m)) * r_e_new_sq

        if (s == 1) then
          ! at r=0; regularity on z-axis
          d_gama_s = 0.d0
          d_gama_m = 0.d0
          d_rho_s  = 0.d0
          d_rho_m  = 0.d0
          d_ww_s   = 0.d0
          d_ww_m   = 0.d0
          d_sphi_s = 0.d0
          d_sphi_m = 0.d0
          d_gama_ss= 0.d0
          d_gama_mm= 0.d0
        else
          d_rho_s  = deriv_s(rho ,s,m)
          d_rho_m  = deriv_m(rho ,s,m)
          d_gama_s = dgds(s,m) ! deriv_s(gama,s,m)
          d_gama_m = dgdm(s,m) ! deriv_m(gama,s,m)
          d_ww_s   = deriv_s(ww  ,s,m)
          d_ww_m   = deriv_m(ww  ,s,m)
          d_sphi_s = deriv_s(sphi,s,m) 
          d_sphi_m = deriv_m(sphi,s,m) 
          d_gama_ss= s1*deriv_s(dgds,s,m) + (1.d0-2.d0*sgp)*d_gama_s
          d_gama_mm= m1*deriv_m(dgdm,s,m) - 2.d0*mum*d_gama_m
        endif

        S_metric_rho(s,m) = e_gsm*( ea/2.d0 * (esm + psm) * s2 * (1.d0+v2sm) / (1.d0-v2sm) &
          + s2 * m1 * e_rsm**2 * ( (s1*d_ww_s)**2 + m1*d_ww_m**2 ) &
          + s1 * d_gama_s - mum * d_gama_m &
          + rsm / 2.d0 * ( (ea*psm-4.d0*Vphi) * s2 - s1*d_gama_s*(s1/2.d0*d_gama_s+1.d0) &
          - d_gama_m*(m1/2.d0*d_gama_m-mum)))

        S_metric_gama(s,m) = e_gsm * ( (ea*psm-4.d0*Vphi) * s2 + gsm/2.d0 * ( (ea*psm-4.d0*Vphi) * s2 &
          - (s1*d_gama_s)**2/2.d0 - m1*d_gama_m**2/2.d0 ) )

        S_metric_omega(s,m) = e_gsm * e_rsm * (-ea*(Omega_c-wwsm)*(esm+psm)*s2/(1.d0-v2sm) &
          + wwsm * ( -0.5d0*ea*s2* (((1.d0+v2sm)*esm + 2.d0*v2sm*psm)/(1.d0-v2sm)) &
          - s1 *(2.d0*d_rho_s+0.5d0*d_gama_s) &
          + mum*(2.d0*d_rho_m+0.5d0*d_gama_m) &
          + 0.25d0*s1**2*(4.d0*d_rho_s**2 - d_gama_s**2) &
          + 0.25d0*m1   *(4.d0*d_rho_m**2 - d_gama_m**2) &
          - m1*e_rsm**2*(sgp**4*d_ww_s**2 + s2*m1*d_ww_m**2) &
          - 2.d0 * Vphi * s2  ) )

        S_metric_sphi(s,m) = - r_e_new_sq * s2 * scal_p * mphi_r &
          + r_e_new_sq * s2 * scal_p * exp(2.d0*alpha(s,m)) * ( -2.d0 * pi * B_coup * (esm-3.d0*psm) + mphi_r) &
          + scal_p * ( &
            - 2.d0 * s1 * sgp * d_gama_s + s1**2 * d_gama_ss + s1**2 * d_gama_s**2 / 4.d0 &
            + s1 * d_gama_s + m1 * d_gama_mm / 2.d0 + d_gama_m**2 * r_e_new_sq * s2 / 4.d0 - 2.d0 * mum * d_gama_m &
          )

      enddo
    enddo

    !--- Angular Integration
    n = 0
    do k = 1, SDIV
      do m = 1, MDIV
        Int_m(m) = P_2n(m,n+1) * S_metric_rho(k,m)
      enddo
      call d01gaf(mu, Int_m, MDIV, sum_rho, er2, ifail)

      do m = 1, MDIV
        Int_m(m) = P_2n(m,n+1) * S_metric_sphi(k,m)
      enddo
      call d01gaf(mu, Int_m, MDIV, sum_sphi, er2, ifail)

      D1_metric_rho  (n+1,k) = sum_rho
      D1_metric_sphi (n+1,k) = sum_sphi
      D1_metric_gama (n+1,k) = 0.d0
      D1_metric_omega(n+1,k) = 0.d0
    enddo

    do n = 1, LMAX
      do k = 1, SDIV
        do m = 1, MDIV
          Int_m(m) = P_2n(m,n+1) * S_metric_rho(k,m)
        enddo
        call d01gaf(mu, Int_m, MDIV, sum_rho, er2, ifail)

        do m = 1, MDIV
          Int_m(m) = P_2n(m,n+1) * S_metric_sphi(k,m)
        enddo
        call d01gaf(mu, Int_m, MDIV, sum_sphi, er2, ifail)

        do m = 1, MDIV
          Int_m(m) = sin_2n_1_theta(m,n) * S_metric_gama(k,m)
        enddo
        call d01gaf(mu, Int_m, MDIV, sum_gama, er2, ifail)

        do m = 1, MDIV
          Int_m(m) = sin_theta(m) * P1_2n_1(m,n+1) * S_metric_omega(k,m)
        enddo
        call d01gaf(mu, Int_m, MDIV, sum_omega, er2, ifail)

        D1_metric_rho  (n+1,k) = sum_rho  
        D1_metric_sphi (n+1,k) = sum_sphi
        D1_metric_gama (n+1,k) = sum_gama
        D1_metric_omega(n+1,k) = sum_omega
        sum_rho   = 0.d0
        sum_sphi  = 0.d0
        sum_gama  = 0.d0
        sum_omega = 0.d0
      enddo
    enddo

    !--- RADIAL INTEGRATION
    n = 0
    do s = 1, SDIV
      do k = 1, SDIV
        Int_s(k) = f_rho(s,n+1,k) * D1_metric_rho(n+1,k)
      enddo
      call d01gaf(s_gp, Int_s, SDIV, sum_rho, er2, ifail)

      do k = 1, SDIV
        mr_s = sqrt(mphi_r*r_e_new_sq) * s_gp(s) / (1.d0-s_gp(s)) 
        mr_k = sqrt(mphi_r*r_e_new_sq) * s_gp(k) / (1.d0-s_gp(k))
        if ( s > k ) then
          if ( k == 1) then 
            f_sphi = exp( -mr_s ) / mr_s * besseli(n*2,mr_k) * besselk(n*2,mr_s)
          else
            f_sphi = ( exp(mr_k-mr_s) - exp(-mr_k-mr_s) ) / 2.d0 / ( mr_s * mr_k ) * besseli(n*2,mr_k) * besselk(n*2,mr_s)
          endif
        else
          if ( s==1 .and. k.ne.1 ) then 
            f_sphi = exp( -mr_k ) / mr_k * besseli(n*2,mr_s) * besselk(n*2,mr_k)
          elseif ( s==1 .and. k==1 ) then 
            f_sphi = 1.d30
          else
            f_sphi = ( exp(mr_s-mr_k) - exp(-mr_s-mr_k) ) / 2.d0 / ( mr_s * mr_k ) * besseli(n*2,mr_s) * besselk(n*2,mr_k)
          endif
        endif
        Int_s(k) = f_sphi / (1.d0-s_gp(k))**2 * sqrt(mphi_r*r_e_new_sq) * D1_metric_sphi(n+1,k)
      enddo
      call d01gaf(s_gp, Int_s, SDIV, sum_sphi, er2, ifail)
      
      D2_metric_rho  (s,n+1) = sum_rho
      D2_metric_sphi (s,n+1) = sum_sphi
      D2_metric_gama (s,n+1) = 0.d0
      D2_metric_omega(s,n+1) = 0.d0
      sum_rho = 0.d0
      sum_sphi= 0.d0
    enddo

    do s = 1, SDIV
      do n = 1, LMAX
        do k = 1, SDIV
          Int_s(k) = f_rho(s,n+1,  k) * D1_metric_rho(n+1,  k)
        enddo
        call d01gaf(s_gp, Int_s, SDIV, sum_rho, er2, ifail)
        
        do k = 1, SDIV
          mr_s = sqrt(mphi_r*r_e_new_sq) * s_gp(s) / (1.d0-s_gp(s)) 
          mr_k = sqrt(mphi_r*r_e_new_sq) * s_gp(k) / (1.d0-s_gp(k))
          if ( s > k ) then
            if ( k == 1) then 
              f_sphi = exp( -mr_s ) / mr_s * besseli(n*2,mr_k) * besselk(n*2,mr_s)
            else
              f_sphi = ( exp(mr_k-mr_s) - exp(-mr_k-mr_s) ) / 2.d0 / ( mr_s * mr_k ) * besseli(n*2,mr_k) * besselk(n*2,mr_s)
            endif
          else
            if ( s==1 .and. k.ne.1 ) then 
              f_sphi = exp( -mr_k ) / mr_k * besseli(n*2,mr_s) * besselk(n*2,mr_k)
            elseif ( s==1 .and. k==1 ) then 
              f_sphi = 1.d30
            else
              f_sphi = ( exp(mr_s-mr_k) - exp(-mr_s-mr_k) ) / 2.d0 / ( mr_s * mr_k ) * besseli(n*2,mr_s) * besselk(n*2,mr_k)
            endif
          endif
          Int_s(k) = f_sphi / (1.d0-s_gp(k))**2 * sqrt(mphi_r*r_e_new_sq) * D1_metric_sphi(n+1,k)
        enddo
        call d01gaf(s_gp, Int_s, SDIV, sum_sphi, er2, ifail)

        do k = 1, SDIV
          Int_s(k) = f_gama(s,n+1,  k)*D1_metric_gama(n+1, k)
        enddo
        call d01gaf(s_gp, Int_s, SDIV, sum_gama, er2, ifail)

        do k = 1, SDIV
          Int_s(k) = merge( f_rho(s,n+1,k)*D1_metric_omega(n+1,k), &
              f_gama(s,n+1,k)*D1_metric_omega(n+1,k), k < s )
        enddo
        call d01gaf(s_gp, Int_s, SDIV, sum_omega, er2, ifail)
        D2_metric_rho  (s,n+1) = sum_rho
        D2_metric_sphi (s,n+1) = sum_sphi
        D2_metric_gama (s,n+1) = sum_gama
        D2_metric_omega(s,n+1) = sum_omega
        sum_rho   = 0.d0
        sum_sphi  = 0.d0
        sum_gama  = 0.d0
        sum_omega = 0.d0
      enddo
    enddo

    sum_rho   = 0.d0
    sum_sphi  = 0.d0
    sum_gama  = 0.d0
    sum_omega = 0.d0
    ! SUMMATION of COEFFICIENTS
    do s = 1, SDIV
      do m = 1, MDIV
        gsm   = gama(s,m)
        rsm   = rho (s,m)
        wwsm  = ww  (s,m)
        e_gsm = exp(-0.5*gsm)
        e_rsm = exp(rsm)
        temp1 = sin_theta(m)

        sum_rho = sum_rho - e_gsm * P_2n(m,1) * D2_metric_rho(s,1)
        sum_sphi= sum_sphi- e_gsm * P_2n(m,1) * D2_metric_sphi(s,1)

        do n = 1, LMAX
          sum_rho = sum_rho - e_gsm * P_2n(m,n+1) * D2_metric_rho(s,n+1)
          sum_sphi= sum_sphi- e_gsm * (2.d0*dble(n)+1.d0) * P_2n(m,n+1) * D2_metric_sphi(s,n+1)
          if (m == MDIV) then
            sum_gama  = sum_gama  - 2.d0 / pi * e_gsm * D2_metric_gama(s,n+1)
            sum_omega = sum_omega + e_rsm*e_gsm*D2_metric_omega(s,n+1)/2.d0
          else
            sum_gama  = sum_gama - 2.d0/pi*e_gsm*(sin_2n_1_theta(m,n)/((2.d0*n-1)*temp1)) &
              * D2_metric_gama(s,n+1)
            sum_omega = sum_omega - e_rsm*e_gsm*(P1_2n_1(m,n+1)/(2.d0*n*(2.d0*n-1)*temp1)) &
              * D2_metric_omega(s,n+1)
          endif
        enddo

        if ( dif < 1.d-6) then
          rho (s,m) = rsm       + cf/2.d0*(sum_rho  - rsm)
          gama(s,m) = gsm       + cf/2.d0*(sum_gama - gsm)
          ww  (s,m) = wwsm      + cf/2.d0*(sum_omega-wwsm)
          sphi(s,m) = sphi(s,m) + cf/2.d0*(sum_sphi - sphi(s,m))
        else
          if ( n_of_it < 100 ) then
            rho (s,m) = rsm       + cf*(sum_rho  - rsm)
            gama(s,m) = gsm       + cf*(sum_gama - gsm)
            ww  (s,m) = wwsm      + cf*(sum_omega-wwsm)
            sphi(s,m) = sphi(s,m) + cf*(sum_sphi - sphi(s,m))
          elseif ( n_of_it < 300 ) then
            rho (s,m) = rsm       + cf/2.d0*(sum_rho  - rsm)
            gama(s,m) = gsm       + cf/2.d0*(sum_gama - gsm)
            ww  (s,m) = wwsm      + cf/2.d0*(sum_omega-wwsm)
            sphi(s,m) = sphi(s,m) + cf/2.d0*(sum_sphi - sphi(s,m))
          else
            rho (s,m) = rsm       + cf/6.d0*(sum_rho  - rsm)
            gama(s,m) = gsm       + cf/6.d0*(sum_gama - gsm)
            ww  (s,m) = wwsm      + cf/6.d0*(sum_omega-wwsm)
            sphi(s,m) = sphi(s,m) + cf/6.d0*(sum_sphi - sphi(s,m))
          endif
        endif
        
        if ( sphi(s,m) .ne. sphi(s,m) ) then
          sphi(s,m) = 0.d0 
        endif

        if ( sphi(s,m) < 0.d0 ) then
          sphi(s,m)= sphi(s-1,m) * &
            exp( sqrt(mphi_r*r_e_new_sq) * (s_gp(s-1) / (1.d0-s_gp(s-1)) - s_gp(s) / (1.d0-s_gp(s)) ) )
        endif

        sum_omega = 0.d0
        sum_rho   = 0.d0
        sum_sphi  = 0.d0
        sum_gama  = 0.d0
      enddo
    enddo

    ! check for divergence
    if (abs(rho(2,1))>100.d0 .or. abs(gama(2,1))>300.d0 .or. abs(ww(2,1))>100.d0 &
        .or. abs(sphi(2,1))>10.d0) then
      write(*,"(30es18.9)") rho(2,1), gama(2,1), ww(2,1), sphi(2,1)
      stop "something diverged; L.352 in spin"
    endif
          
    if (r_ratio == 1.d0) then
      do s = 1, SDIV
        do m = 1, MDIV
          rho(s,m) = rho(s,1)
          sphi(s,m)= sphi(s,1)
          gama(s,m)= gama(s,1)
          ww(s,m)  = 0.d0
        enddo
      enddo
    endif
      
    ! --- the fourth equation

    ! compute first order derivatives of gama

    ! alpha
    alpha(:,:) = 0.d0
    if (r_ratio == 1.d0) then
      da_dm(:,:) = 0.0
    else
      do s = 2, SDIV
        do m = 1, MDIV
          da_dm(1,m) = 0.0
          sgp = s_gp(s)
          s1  = sgp * (1.d0-sgp)
          mum = mu(m)
          m1  = 1.d0 - mum**2
          d_gama_s  = dgds(s,m)
          d_gama_m  = dgdm(s,m)
          d_rho_s   = deriv_s(rho,s,m)
          d_rho_m   = deriv_m(rho,s,m)
          d_sphi_s  = deriv_s(sphi,s,m) 
          d_sphi_m  = deriv_m(sphi,s,m)
          d_gama_sm = deriv_sm(gama,s,m)
                      
          d_ww_s    = deriv_s(ww,s,m)
          d_ww_m    = deriv_m(ww,s,m)
                      
          d_gama_ss = s1*deriv_s(dgds,s,m) + (1.d0-2.d0*sgp)*d_gama_s
          d_gama_mm = m1*deriv_m(dgdm,s,m) - 2.d0*mum*d_gama_m

          temp1 = 2.d0 * sgp**2 * (sgp/(1.d0-sgp)) * m1 * d_ww_s * d_ww_m * (1.d0+s1*d_gama_s) &
            - ( (sgp**2 * d_ww_s)**2 - (sgp*d_ww_m/(1.d0-sgp))**2*m1 ) * (-mum + m1*d_gama_m)

          temp2 = 1.d0/( m1 * (1.d0+s1*d_gama_s)**2 + (-mum+m1*d_gama_m)**2 )

          temp3 = s1*d_gama_ss + (s1*d_gama_s)**2

          temp4 = d_gama_m*(-mum + m1*d_gama_m)

          temp5 = ( (s1*(d_rho_s+d_gama_s))**2 - m1*(d_rho_m+d_gama_m)**2 ) &
            * (-mum + m1*d_gama_m)

          temp6 = s1*m1*(  (d_rho_s+d_gama_s)*(d_rho_m+d_gama_m)/2.d0 &
            + d_gama_sm + d_gama_s*d_gama_m  ) * (1.d0 + s1*d_gama_s)

          temp7 = s1 * mum * d_gama_s * ( 1.d0 + s1 * d_gama_s )

          temp8 = m1 * exp( -2.d0 * rho(s,m) )

          temp9 = -temp2*(-mum+m1*d_gama_m)*( (s1*d_sphi_s)**2 - m1*d_sphi_m**2 ) &
                - m1 * s1 * ( 1.d0 + s1 * d_gama_s ) * 2.d0 * d_sphi_m * d_sphi_s
                
          da_dm(s,m) = - (d_rho_m+d_gama_m)/2.d0 &
            - temp2*( (temp3 - d_gama_mm - temp4)*(-mum+m1*d_gama_m)/2.d0 &
            + temp5/4.d0 - temp6  + temp7 + temp8*temp1/4.d0 ) + temp9

        enddo
      enddo
    endif

    do s = 1, SDIV
      alpha(s,1) = 0.d0
      do m = 1, MDIV-1
        alpha(s,m+1) = alpha(s,m) + dm * ( da_dm(s,m+1) + da_dm(s,m) ) / 2.d0
      enddo
      do m = 4, MDIV
        call d01gaf( mu(1:m), da_dm(s,1:m), m, alpha(s,m), er2, ifail)
        !alpha(s,m+1) = alpha(s,m) + dm * ( da_dm(s,m+1) + da_dm(s,m) ) / 2.d0
      enddo
    enddo

    do s = 1, SDIV
      do m = 1, MDIV
        alpha(s,m) = alpha(s,m) - alpha(s,MDIV) + ( gama(s,MDIV)-rho(s,MDIV) )/2.d0
        if(alpha(s,m).ge.300.0) then
          write(*,*) s,m
          stop "L392, alpha fails"
        endif
        ww(s,m) = ww(s,m) / r_e_new
      enddo
    enddo
#if 0
    open(98,file="./Debug/1d.dat")
    do s = 1, SDIV
        if (enthalpy(s,1) > enthalpy_min) then 
        rho_0 = n0_at_e( energy(s,1) ) * MB
        else 
        rho_0 = 0.d0
        endif
        write(98,"(99es27.17e3)") s_gp(s), alpha(s,1), gama(s,1), rho(s,1), ww(s,1) * (C/sqrt(kappa)), & ! 1-6
        pressure(s,1)/KSCALE, energy(s,1)/(C*C*KSCALE), enthalpy(s,1), rho_0, & ! 7-10
        velocity_sq(s,1), omg(s,1) * (C/sqrt(kappa)), sphi(s,1)*sqrt(B_coup) ! 11-13
    enddo
    close(98)
#endif
    n_of_it = n_of_it + 1
    if (n_of_it == 1500 .and. dif > 1.d-6 ) stop "Cannot converge; L404 in spin"
  enddo
  ! --- End of iteration 

  ! compute omega
  omg(:,:) = Omega_c / r_e_new
  Omega_c  = Omega_c / r_e_new
  Omega_e  = Omega_c
  r_e      = r_e_new
  sphi_c   = sphi(1,1) * sqrt(B_coup) 
  rho_0    = n0_at_e( energy(1,1) ) * MB

  if (output) then
    call mass_radius
    write(fil1,"(f6.2)") ang_mom
    write(fil2,"(f16.5)") mass_0/MSUN
    write(fil3,"(es15.2)") B_coup
    write(fil4,"(es15.2)") sqrt(mphi_r*1.d10/KAPPA)*l_uni
    write(fil5,"(es15.3)") rho_0
    write(fil6,"(f15.3)") sphi_c

    open(98,file="./Cont/"//trim(adjustl(eos_file))//"_J_"//trim(adjustl(fil1))//&
      "_Mb"//trim(adjustl(fil2))//"_B"//trim(adjustl(fil3))//&
      "_mphi"//trim(adjustl(fil4))//"_rhoc"//trim(adjustl(fil5))//"_sphic"//trim(adjustl(fil6))//".dat")
    !open(98,file="./Cont/test.dat")
    write(98,"(2i5,99es27.17e3)") SDIV, MDIV, r_e*sqrt(KAPPA)/1.d5, &
            energy(1,1)/(C*C*KSCALE), r_ratio, Omega_e* (C/sqrt(kappa)) , Omega_c* (C/sqrt(kappa)) 
    do s = 1, SDIV
      do m = 1, MDIV
        ! r, \theta, \apha, \gamma, \rho, \omega, \phi, \varepsilon, \rho_0, p
        if (enthalpy(s,m) > enthalpy_min) then 
          rho_0 = n0_at_e( energy(s,m) ) * MB
        else 
          rho_0 = 0.d0
        endif
        write(98,"(99es27.17e3)") s_gp(s), mu(m), alpha(s,m), gama(s,m), rho(s,m), ww(s,m) * (C/sqrt(kappa)), & ! 1-6
          pressure(s,m)/KSCALE, energy(s,m)/(C*C*KSCALE), enthalpy(s,m), rho_0, & ! 7-10
          velocity_sq(s,m), omg(s,m) * (C/sqrt(kappa)), sphi(s,m)*sqrt(B_coup) ! 11-13
      enddo
    enddo
    close(98)

    open(99,file="./Res/res.dat")
    write(99,"(2i5,99es27.17e3)") SDIV, MDIV, r_e*sqrt(KAPPA)/1.d5, &
            energy(1,1)/(C*C*KSCALE), r_ratio, Omega_e* (C/sqrt(kappa)) , Omega_c* (C/sqrt(kappa)) 
    do s = 1, SDIV
      do m = 1, MDIV
        ! r, \theta, \apha, \gamma, \rho, \omega, \phi, \varepsilon, \rho_0, p
        if (enthalpy(s,m) > enthalpy_min) then 
          rho_0 = n0_at_e( energy(s,m) ) * MB
        else 
          rho_0 = 0.d0
        endif
        write(99,"(99es27.17e3)") s_gp(s), mu(m), alpha(s,m), gama(s,m), rho(s,m), ww(s,m) * (C/sqrt(kappa)), & ! 1-6
          pressure(s,m)/KSCALE, energy(s,m)/(C*C*KSCALE), enthalpy(s,m), rho_0, & ! 7-10
          velocity_sq(s,m), omg(s,m) * (C/sqrt(kappa)), sphi(s,m)*sqrt(B_coup) ! 11-13
      enddo
    enddo
    close(99)
  endif

end subroutine spin 
