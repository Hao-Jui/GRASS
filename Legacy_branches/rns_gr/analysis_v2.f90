subroutine mass_radius
  ...
  if (output) then
    ...
    write(fil3,"(f6.4)") chi
    open(21, file="./Cont/Omega"//"_parA"//trim(adjustl(fil1))//"_parB"//trim(adjustl(fil2))//&
        "_chi"//trim(adjustl(fil3))//".dat")
    do s = 1, 2*SDIV/3
  ...
  do s = 1 , SDIV
    ...
    dd_r(s)   = deriv_s(d_r_e,s,1)
    dd_g(s)   = deriv_s(d_g_e,s,1)
    dd_o(s)   = deriv_s(d_o_e,s,1)
