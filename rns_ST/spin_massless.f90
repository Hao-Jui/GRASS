...
#include "option_macro.h"
  ...
  do while( dif > 1.d-7 .or. n_of_it < 2 )
      ...
      if ( mod(n_of_it,10)==0 ) write(*,"(es15.6,A10,20es18.9)") &
            r_e_new, "--->", dif, sphi_center_h*r_e_new*sqrt(B_coup)
      ...
      do s = 1, SDIV
        do m = 1, MDIV
          sgp   = s_gp(s)
          scal_p= sphi(s,m) 
          Acoup = exp(- scal_p**2 * B_coup / 4.d0 )
          rsm   = rho     (s,m)
          gsm   = gama    (s,m)
          wwsm  = ww      (s,m) ! hat
          ...
          S_metric_omega(s,m) = e_gsm * e_rsm * (-ea*(Omega_c-wwsm)*(esm+psm)*s2/(1.d0-v2sm) &
            + wwsm * (-0.5*ea*s2*(((1.d0+v2sm)*esm + 2.d0*v2sm*psm)/(1.d0-v2sm)) &
          ...
          S_metric_sphi(s,m) = -s1**2 * d_gama_s * d_sphi_s - m1 * d_gama_m * d_sphi_m &
            + r_e_new_sq * s2 * exp(2.d0*alpha(s,m)) * scal_p * ( -2.d0 * pi * B_coup * (esm-3.d0*psm) )
        enddo
      enddo
      ...
      enddo
      !write(*,"(12es15.6)") D1_metric_sphi(1,SDIV-3:SDIV)
      ...
      do s = 1, SDIV
        do m = 1, MDIV
          ...
          sum_sphi= sum_sphi- P_2n(m,1) * D2_metric_sphi(s,1)
    ...
    ! compute first order derivatives of gama
      ...
      if (n_of_it == 10000) stop "Cannot converge; L404 in spin"
...
! --- End of iteration 
  ...
  if (output) then
    ...
    write(fil3,"(f6.2)") B_coup
    write(fil4,"(f6.2)") mphi_
    write(fil5,"(es15.3)") rho_0
    write(fil6,"(f15.3)") sphi_c
    
    open(98,file="./Cont/J_"//trim(adjustl(fil1))//"_Mb"//trim(adjustl(fil2))//"_B"//trim(adjustl(fil3))//&
